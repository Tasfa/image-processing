...
//-------------------------------实验一：bmp格式图像文件的读入与显示，24位真彩图像转换为灰度图像------------------------------------/
...
BOOL LoadBmpFile (char* BmpFileName)
{
	...
	//LONG size= sizeof(BITMAPINFOHEADER) +bi.biHeight*bi.biWidth*3;
...
//-------------巴特沃斯频域滤波函数-----------------//
void ba_Filter(int D){//D=0 低通 1 高通, n为阶
	int n=1;
	//图像中心
	...
	if(D){
		...
		for (i = 0; i < FFT_h; i ++)
		{
			for (j = 0; j < FFT_w; j ++)
			{
				d2 = pow(i-centerWidth,2)+pow(j-centerWidth,2);
				gFD[i + FFT_h * j]*=1.0/(1+pow(r2/d2,n)) + 0.8;
			}
		}
		...
		for (i = 0; i < FFT_h; i ++)
		{
			...
			{
				d2 = pow(i-centerWidth,2)+pow(j-centerWidth,2);
				gFD[i + FFT_h * j]*=1.0/(1+pow(d2/r2,n));
			}
		}
	}
